...
private struct EagleRainbowSpinner: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 30.0, paused: reduceMotion)) { context in
            let phase = reduceMotion
                ? 0.0
                : context.date.timeIntervalSinceReferenceDate.truncatingRemainder(dividingBy: 1.0)

            ZStack {
                ForEach(0..<12, id: \.self) { index in
                    Capsule(style: .continuous)
                        .fill(Color(
                            hue: (Double(index) / 12.0 + phase).truncatingRemainder(dividingBy: 1.0),
                            saturation: 0.9,
                            brightness: 1
                        ))
                        .frame(width: 2.4, height: 6.2)
                        .offset(y: -6.2)
                        .rotationEffect(.degrees(Double(index) * 30 + (reduceMotion ? 0 : phase * 360)))
                        .opacity(0.35 + (Double(index + 1) / 12.0) * 0.65)
                }
            }
...
private struct EagleRainbowProgressBar: View {
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    let value: Double
    ...
    var body: some View {
        ...
        .accessibilityLabel(LaraL10n.text(en: "Preparation progress", es: "Progreso de preparación"))
